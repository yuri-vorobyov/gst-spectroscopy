...
2. Run the script.
3. Once the plot window appears --- use mouse to select the linear range. Approximation results will be displayed.
...
# A hack below. Matplotlib keeps redrawing plot extending the viewport to the x = 0 after
# approximation line is added. Two lines below prevent such behavior.
x_min, _ = ax.get_xlim()
...
def onselect(xmin, xmax):
    ...
    if this.vspan:
        this.vspan.remove()
    this.vspan = ax.axvspan(xmin, xmax, facecolor='green', alpha=0.2, linewidth=1.0, edgecolor='darkgreen')
    ...
    if this.text:
        this.text.remove()
    this.text = ax.text(0.05, 0.85, f'$ E_g \\, = \\, {-res[1] / res[0]:.3f} \\; \\mathrm{{eV}} $', transform=ax.transAxes)
